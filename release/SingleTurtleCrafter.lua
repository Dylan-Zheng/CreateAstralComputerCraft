...
modules["programs.SingleTurtleCrafter"] = function(...) local cc=require("libraries.basalt")
local dc=require("elements.TabView")local _d=require("utils.Logger")
local ad=require("programs.common.SnapShot")
local bd=require("programs.crafter.positive.RecipeManager")local cd=require("wrapper.PeripheralWrapper")
local dd=require("programs.crafter.single.RecipeTab")local __a=require("programs.crafter.single.SettingTab")
local a_a=require("programs.crafter.positive.TurtleCraft")
local b_a=require("programs.crafter.single.SettingManager")local c_a=false;cc.LOGGER.setEnabled(c_a)
cc.LOGGER.setLogToFile(c_a)
if c_a then
_d.addPrintFunction(function(_ba,aba,bba,cba)cba=string.format("[%s:%d] %s",aba,bba,cba)
if
_ba==_d.levels.DEBUG then cc.LOGGER.debug(cba)elseif
_ba==_d.levels.INFO then cc.LOGGER.info(cba)elseif _ba==_d.levels.WARN then
cc.LOGGER.warn(cba)elseif _ba==_d.levels.ERROR then cc.LOGGER.error(cba)end end)end;_d.currentLevel=_d.levels.ERROR;cd.reloadAll()bd.load()
ad.takeSnapShot()local d_a=cc.getMainFrame()
local _aa=dc:new(d_a:addFrame(),1,1,d_a:getWidth(),d_a:getHeight())local aaa=_aa:createTab("RecipesTab")
dd:new(aaa.frame,1,1,aaa.frame:getWidth(),aaa.frame:getHeight()):init()local baa=_aa:createTab("Settings")
__a:new(baa.frame):init()_aa:init()local caa=bd.getRecipes()
local daa=function()local _ba=1
while true do if bd.checkedUpdate()then
caa=bd.getRecipes()end;for aba,bba in pairs(caa)do
if
...
modules["programs.crafter.single.RecipeTab"] = function(...) local cb=require("libraries.basalt")
local db=require("utils.Logger")local _c=require("elements.ItemSelectedListBox")
local ac=require("programs.common.SnapShot")local bc=require("programs.common.TriggerTurtleView")
local cc=require("elements.MessageBox")local dc=require("elements.ConfirmMessageBox")
local _d=require("elements.ScrollableFrame")
local ad=require("programs.crafter.positive.RecipeManager")
local function bd(__a)if not __a then return __a end;local a_a=string.find(__a,":")if a_a then return string.sub(__a,
a_a+1)else return __a end end
local cd=function(__a,a_a)local b_a={}if __a then
table.insert(b_a,{text=__a,selected=true})end;for c_a,d_a in pairs(a_a)do if __a~=c_a then
table.insert(b_a,{text=c_a})end end;return b_a end;local dd={}dd.__index=dd
function dd:new(__a,a_a,b_a,c_a,d_a)local _aa=setmetatable({},dd)
_aa.selectedRecipe=nil;_aa._displayList={}_aa.pframe=__a;_aa.selectedCrafting=nil
_aa.onSelectedCallback=function()end;_aa.onNewCallback=function()end
_aa.getDisplayCraftingListFn=function()return{}end
_aa.innerFrame=_aa.pframe:addFrame():setPosition(a_a,b_a):setSize(c_a,d_a):setBackground(colors.lightGray):setForeground(colors.white)
_aa.searchInput=_aa.innerFrame:addInput():setPosition(2,2):setSize(12,1):setBackground(colors.gray):setForeground(colors.white):setPlaceholderColor(colors.lightGray):setPlaceholder("Search..."):onChange("text",function(caa,daa)
_aa:refreshCraftingList(daa)end)
_aa.clearBtn=_aa.innerFrame:addButton():setPosition(
_aa.searchInput:getX()+_aa.searchInput:getWidth()+1,_aa.searchInput:getY()):setSize(1,1):setText("C"):setBackground(colors.gray):setForeground(colors.white):onClick(function()
_aa.searchInput:setText("")_aa:refreshCraftingList()end)
_aa.craftingList=_aa.innerFrame:addList():setPosition(2,
_aa.searchInput:getY()+_aa.searchInput:getHeight()+1):setSize(14,
_aa.innerFrame:getHeight()- (
_aa.searchInput:getY()+_aa.searchInput:getHeight()+3)):setBackground(colors.gray):setForeground(colors.white):onSelect(function(caa,daa,_ba)
if

_aa.selectedRecipe and _aa.selectedRecipe.output==_ba.recipe.output then _ba.selected=false;_aa.selectedRecipe=nil;_aa:setDetails(nil)
_aa.craftingList:render()else _aa:setDetails(_ba.recipe)end end)
_aa.newButton=_aa.innerFrame:addButton():setPosition(_aa.craftingList:getX(),
_aa.craftingList:getY()+_aa.craftingList:getHeight()+1):setSize(5,1):setText("New"):setBackground(colors.gray):setForeground(colors.white):onClick(function()_aa:setDetails(
nil)end)
_aa.deleteButton=_aa.innerFrame:addButton():setPosition(
_aa.newButton:getX()+_aa.newButton:getWidth()+1,_aa.newButton:getY()):setSize(5,1):setText("Del"):setBackground(colors.gray):setForeground(colors.white):onClick(function()
if
_aa.selectedRecipe then
_aa.confirmMessageBox:open("Delete Recipe","Are you sure you want to delete the selected recipe .. ".. (
_aa.selectedRecipe.output or"empty").." ?",function()
ad.removeRecipe(_aa.selectedRecipe)
_aa.craftingList:setItems(_aa:refreshCraftingList(_aa.searchInput:getText()))_aa:setDetails(nil)end)else
_aa.messageBox:open("Error","No recipe selected to delete.")end end)
_aa.detailFrame=_aa.innerFrame:addFrame():setPosition(
_aa.craftingList:getX()+_aa.craftingList:getWidth()+1,2):setSize(
_aa.innerFrame:getWidth()- (
_aa.craftingList:getX()+_aa.craftingList:getWidth()+1),
_aa.innerFrame:getHeight()-2):setBackground(colors.gray):setForeground(colors.white)
_aa.outputLabel=_aa.detailFrame:addLabel():setPosition(2,2):setSize(
_aa.detailFrame:getWidth()-3,1):setBackground(colors.gray):setForeground(colors.white):setText("Output: ")
_aa.outputValueLabel=_aa.detailFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(2,
_aa.outputLabel:getY()+2):setSize(
_aa.detailFrame:getWidth()-3,1):setText("empty"):setBackground(colors.lightGray):setForeground(colors.white):onClick(function()
_aa.itemListBox:open(cd(
_aa.selectedRecipe and _aa.selectedRecipe.output or nil,ac.items),false,{confirm=function(caa)_aa.selectedRecipe=
_aa.selectedRecipe or{}
_aa.selectedRecipe.output=caa[1].text;_aa:setDetails(_aa.selectedRecipe)end})end)
_aa.inputInvLabel=_aa.detailFrame:addLabel():setPosition(2,
_aa.outputValueLabel:getY()+_aa.outputValueLabel:getHeight()+1):setSize(
_aa.detailFrame:getWidth()-3,1):setBackground(colors.gray):setForeground(colors.white):setText("Input: ")local aaa={"i1","i2","i3","i4","i5","i6","i7","i8","i9"}
local baa=
_aa.inputInvLabel:getY()+_aa.inputInvLabel:getHeight()+1
for i=1,9 do
_aa.detailFrame:addLabel():setPosition(2,baa):setBackgroundEnabled(true):setPosition(2,baa):setSize(
_aa.detailFrame:getWidth()-3,1):setBackground(colors.lightGray):setForeground(colors.white):setText(
i..". ")
_aa[aaa[i]]=_aa.detailFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(5,baa):setSize(
_aa.detailFrame:getWidth()-6,1):setBackground(colors.lightGray):setForeground(colors.white):setText("empty"):onClick(function()
_aa.itemListBox:open(cd(
_aa.selectedRecipe and _aa.selectedRecipe.inputs and
_aa.selectedRecipe.inputs[i]or nil,ac.items),false,{confirm=function(caa)_aa.selectedRecipe=
_aa.selectedRecipe or{}_aa.selectedRecipe.inputs=
_aa.selectedRecipe.inputs or{}if
not caa[1]or not caa[1].text then _aa.selectedRecipe.inputs[i]=false else
_aa.selectedRecipe.inputs[i]=
caa[1].text and caa[1].text or nil end
_aa:setDetails(_aa.selectedRecipe)end})end)
baa=_aa[aaa[i]]:getY()+_aa[aaa[i]]:getHeight()end
_aa.maxInputsLabel=_aa.detailFrame:addLabel():setPosition(2,baa+1):setForeground(colors.white):setText("Max: ")
_aa.maxInputBox=_aa.detailFrame:addInput():setPosition(
_aa.maxInputsLabel:getX()+_aa.maxInputsLabel:getWidth()+1,_aa.maxInputsLabel:getY()):setSize(
_aa.detailFrame:getWidth()- (
_aa.maxInputsLabel:getX()+_aa.maxInputsLabel:getWidth()+2),1):setBackground(colors.lightGray):setForeground(colors.black):setText("0"):setPattern("[0-9]")
_aa.triggerLabel=_aa.detailFrame:addLabel():setPosition(2,
_aa.maxInputsLabel:getY()+_aa.maxInputBox:getHeight()+1):setText("Trigger:"):setForeground(colors.white)
_aa.editTriggerBtn=_aa.detailFrame:addButton():setPosition(
_aa.triggerLabel:getX()+_aa.triggerLabel:getWidth()+2,_aa.triggerLabel:getY()):setSize(4,1):setText("Edit"):setBackground(colors.lightGray):setForeground(colors.black):onClick(function()
_aa.triggerBox:open(
_aa.selectedRecipe and _aa.selectedRecipe.trigger or nil,function(caa)_aa.selectedRecipe=
_aa.selectedRecipe or{}_aa.selectedRecipe.trigger=caa end)end)
_aa.clearTriggerBtn=_aa.detailFrame:addButton():setPosition(
_aa.editTriggerBtn:getX()+_aa.editTriggerBtn:getWidth()+1,_aa.editTriggerBtn:getY()):setSize(4,1):setText("Clr"):setBackground(colors.lightGray):setForeground(colors.black):onClick(function()if
_aa.selectedRecipe then _aa.selectedRecipe.trigger=nil end end)
_aa.saveBtn=_aa.detailFrame:addButton():setPosition(
_aa.detailFrame:getWidth()-5,_aa.clearTriggerBtn:getY()+
_aa.clearTriggerBtn:getHeight()+1):setSize(4,1):setText("Save"):setBackground(colors.green):setForeground(colors.black):onClick(function()
if
_aa.selectedRecipe then
local caa=tonumber(_aa.maxInputBox:getText())or 0;if caa<0 or caa>64 then
_aa.messageBox:open("Error","Max Inputs must be between 0 and 64.")return end
_aa.selectedRecipe.maxInputs=caa;local daa,_ba=ad.addRecipe(_aa.selectedRecipe)
if daa then
_aa:refreshCraftingList(_aa.searchInput:getText())
_aa.messageBox:open("Success","Recipe saved successfully.")else
_aa.messageBox:open("Error","Error saving recipe: ".. (_ba or"unknown error"))end else
_aa.messageBox:open("Error","No recipe selected to save.")end end)
_aa.detailFrame:addLabel():setPosition(2,
_aa.saveBtn:getY()+1):setText("")
_d.setScrollable(_aa.detailFrame,true,colors.gray,colors.lightGray,colors.gray,colors.white)_aa.itemListBox=_c:new(_aa.pframe)
_aa.triggerBox=bc:new(_aa.pframe)_aa.messageBox=cc:new(_aa.pframe)
_aa.confirmMessageBox=dc:new(_aa.pframe)return _aa end
function dd:getDisplayList()
if ad.checkedUpdate()then local __a=ad.getRecipes()
self._displayList={}for a_a,b_a in pairs(__a)do
table.insert(self._displayList,{text=bd(a_a),recipe=b_a})end end;return self._displayList end
function dd:refreshCraftingList(__a)local a_a=self:getDisplayList()local b_a={}
if __a and __a~=""then
local c_a=string.lower(__a)
for d_a,_aa in ipairs(a_a)do if string.find(string.lower(_aa.text),c_a,1,true)then
table.insert(b_a,_aa)end end else for c_a,d_a in ipairs(a_a)do table.insert(b_a,d_a)end end;self.craftingList:setItems(b_a)end
function dd:setDetails(__a)
if __a then self.selectedRecipe=__a
self.outputValueLabel:setText(__a.output or"empty")local a_a=__a.inputs or{}for i=1,9 do local b_a=self["i"..i]local c_a=nil;if a_a[i]then
c_a=bd(a_a[i])end
if b_a then b_a:setText(c_a or"empty")end end
self.maxInputBox:setText(tostring(
__a.maxInputs or 0))else self.selectedRecipe={inputs={},output=nil,trigger=nil,maxInputs=64}
self.outputValueLabel:setText("empty")
for i=1,9 do local a_a=self["i"..i]if a_a then a_a:setText("empty")end end;self.maxInputBox:setText("64")end end;function dd:init()self:refreshCraftingList()
self:setDetails(nil)return self end;return dd end
modules["programs.crafter.single.SettingTab"] = function(...) local ca=require("utils.Logger")
local da=require("utils.OSUtils")local _b=require("wrapper.PeripheralWrapper")
local ab=require("elements.ItemSelectedListBox")
local bb=require("programs.crafter.single.SettingManager")local cb=colors;local db={}db.__index=db
function db:new(_c)local ac=setmetatable({},db)
ac.pframe=_c
...
ac.recipesChestTitleLabel=ac.recipesChestSettingFrame:addLabel():setText("Inventory Setting"):setPosition(2,2):setBackground(cb.gray):setForeground(cb.white)
ac.storageLabel=ac.recipesChestSettingFrame:addLabel():setText("Storage:"):setPosition(2,4):setBackground(cb.gray):setForeground(cb.white)
ac.storageValueLabel=ac.recipesChestSettingFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(
ac.storageLabel:getX()+ac.storageLabel:getWidth()+1,ac.storageLabel:getY()):setSize(
ac.recipesChestSettingFrame:getWidth()- (
ac.storageLabel:getX()+ac.storageLabel:getWidth()+1),1):setText("..."):setBackground(cb.lightGray):setForeground(cb.white):onClick(function()
ac:openChestSelector("storage")end)
ac.bufferLabel=ac.recipesChestSettingFrame:addLabel():setText("Buffer:"):setPosition(2,
ac.storageLabel:getY()+ac.storageLabel:getHeight()):setBackground(cb.gray):setForeground(cb.white)
ac.bufferValueLabel=ac.recipesChestSettingFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(
ac.bufferLabel:getX()+ac.bufferLabel:getWidth()+1,ac.bufferLabel:getY()):setSize(
ac.recipesChestSettingFrame:getWidth()- (
ac.bufferLabel:getX()+ac.bufferLabel:getWidth()+1),1):setText("..."):setBackground(cb.lightGray):setForeground(cb.white):onClick(function()
ac:openChestSelector("buffer")end)
ac.saveButton=ac.innerFrame:addButton():setText("Save"):setPosition(
ac.innerFrame:getWidth()-7,ac.innerFrame:getHeight()-2):setSize(6,1):setBackground(cb.green):setForeground(cb.white)ac.itemListBox=ab:new(ac.pframe)return ac end
function db:init()local _c=bb.getSettings()
ca.debug("Loaded settings: {}",_c)
if _c.storage then
self.storageValueLabel:setText(_c.storage)else self.storageValueLabel:setText("...")end
if _c.buffer then
self.bufferValueLabel:setText(_c.buffer)else self.bufferValueLabel:setText("...")end
self.saveButton:onClick(function()
bb.setSettings({storage=
self.storageValueLabel:getText()~=
"..."and self.storageValueLabel:getText()or nil,buffer=

self.bufferValueLabel:getText()~="..."and self.bufferValueLabel:getText()or nil})os.reboot()end)end
function db:openChestSelector(_c)_b.reloadAll()local ac=_b.getAll()local bc={}local cc;if
_c=="storage"then cc=self.storageValueLabel:getText()elseif _c=="buffer"then
cc=self.bufferValueLabel:getText()end;for dc,_d in pairs(ac)do
table.insert(bc,{text=dc,selected=(
dc==cc and cc~="...")})end
self.itemListBox:open(bc,false,{confirm=function(dc)
if
dc and#dc>0 then if _c=="storage"then
self.storageValueLabel:setText(dc[1].text)elseif _c=="buffer"then
self.bufferValueLabel:setText(dc[1].text)end end end})end;return db end
...
local ac={1,2,3,5,6,7,9,10,11}local bc={}local cc=cb.wrap("front")local dc=cb.wrap("bottom")bc.setStorage=function(__a)
cc=__a end;bc.setBuffer=function(__a)dc=__a end
local _d=function()
...
modules["programs.crafter.single.SettingManager"] = function(...) local d=require("utils.OSUtils")
local _a=require("utils.Logger")local aa={settings=nil}
aa.getSettings=function()
if not aa.settings then aa.settings=d.loadTable("Settings")or
{storage=nil,buffer=nil}end;return aa.settings end
aa.setSettings=function(ba)aa.settings=ba;d.saveTable("Settings",aa.settings)end;return aa end
...
modules["programs.common.TriggerTurtleView"] = function(...) local ac=require("utils.StringUtils")
local bc=require("wrapper.PeripheralWrapper")local cc=require("utils.OSUtils")
local dc=require("utils.Logger")local _d=require("elements.ItemSelectedListBox")
local ad=require("programs.common.Trigger")local bd=require("programs.common.SnapShot")
local cd=require("elements.MessageBox")local dd=require("elements.ConfirmMessageBox")
local __a=require("elements.ScrollableFrame")local a_a=textutils;local b_a=colors;local c_a={}c_a.__index=c_a;local d_a=ad.TYPES
local _aa=ad.CONDITION_TYPES
...
local daa=baa.frame:getHeight()local _ba=math.floor(caa*1 /3)-1;local aba=caa-_ba-2
baa.conditionTree=baa.frame:addTree():setPosition(2,2):setSize(
_ba-1,daa-4):setBackground(b_a.gray):setForeground(b_a.white):setSelectedColor(b_a.lightBlue):onSelect(function(bba,cba)if
cba and cba.data then baa:updateDetails(cba.data)
baa.selectedTreeNode=cba end end)
baa.nodeDetailsFrame=baa.frame:addFrame():setPosition(_ba+2,2):setSize(aba,
...
aba-baa.parentNodeLabel:getWidth()-6,1):setBackground(b_a.lightGray):setForeground(b_a.white):setText("root")
baa.changedParentNodeBtn=baa.nodeDetailsFrame:addButton():setPosition(
baa.nodeDetailsFrame:getWidth()-3,2):setSize(3,1):setBackground(b_a.lightGray):setForeground(b_a.white):setText("..."):onClick(function()
...
baa.triggerTypeDropdown:getY()+2):setBackground(b_a.gray):setForeground(b_a.white):setText("Item:")
...
baa.nodeDetailsFrame:getWidth()-baa.itemLabel:getWidth()-6,1):setBackground(b_a.black):setForeground(b_a.white):setText("")
baa.selectItemBtn=baa.nodeDetailsFrame:addButton():setPosition(
baa.nodeDetailsFrame:getWidth()-3,baa.itemLabel:getY()):setSize(3,1):setBackground(b_a.lightGray):setForeground(b_a.white):setText("..."):onClick(function()
...
baa.amountLabel:getX()+baa.amountLabel:getWidth()+1,baa.amountLabel:getY()):setSize(8,1):setBackground(b_a.lightGray):setForeground(b_a.white):setText("0"):setPlaceholder("0"):setPattern("[0-9]")
baa.modifyNodeBtn=baa.nodeDetailsFrame:addButton():setPosition(
baa.nodeDetailsFrame:getWidth()-18,baa.amountInput:getY()+2):setSize(5,1):setBackground(b_a.blue):setForeground(b_a.white):setText("Mod"):setVisible(false):onClick(function()
baa:modifySelectedNode()end)
baa.addNodeBtn=baa.nodeDetailsFrame:addButton():setPosition(
baa.nodeDetailsFrame:getWidth()-12,baa.amountInput:getY()+2):setSize(4,1):setBackground(b_a.green):setForeground(b_a.white):setText("Add"):onClick(function()
baa:addNodeToTree()end)
baa.nodeDetailsFrame:addLabel():setPosition(2,
baa.addNodeBtn:getY()+1):setText("")
baa.deleteNodeBtn=baa.nodeDetailsFrame:addButton():setPosition(
baa.nodeDetailsFrame:getWidth()-7,baa.addNodeBtn:getY()):setSize(5,1):setBackground(b_a.red):setForeground(b_a.white):setVisible(false):setText("Del"):onClick(function()
baa:deleteSelectedNode()end)
baa.NewTriggerBtn=baa.frame:addButton():setPosition(2,
baa.conditionTree:getY()+baa.conditionTree:getHeight()+1):setSize(1,1):setBackground(b_a.red):setForeground(b_a.white):setText("N"):onClick(function()
...
baa.conditionTree:getY()+baa.conditionTree:getHeight()+1):setSize(1,1):setBackground(b_a.green):setForeground(b_a.white):setText("S"):onClick(function()
...
baa.conditionTree:getY()+baa.conditionTree:getHeight()+1):setSize(1,1):setBackground(b_a.gray):setForeground(b_a.white):setText("B"):onClick(function()
baa:close()end)baa.itemListBox=_d:new(baa.pframe)
baa.messageBox=cd:new(baa.frame,20,8)baa.confirmMessageBox=dd:new(baa.frame,20,8)
__a.setScrollable(baa.nodeDetailsFrame,true,b_a.gray,b_a.lightGray,b_a.gray,b_a.white)baa:updateUIForTriggerType()return baa end
...
if aaa==
d_a.ITEM_COUNT then for daa,_ba in pairs(bd.items)do
table.insert(baa,{text=daa,data={type="item",name=daa}})end elseif aaa==d_a.FLUID_COUNT then for daa,_ba in
pairs(bd.fluids)do
table.insert(baa,{text=daa,data={type="fluid",name=daa}})end end
...
self.amountInput:setText("0")self:setTriggerType(d_a.ITEM_COUNT)
self:setConditionType(_aa.COUNT_GREATER)self:updateUIForTriggerType()
self.modifyNodeBtn:setVisible(false)self.deleteNodeBtn:setVisible(false)else
self.selectNodeData={parentNodeId=
aaa.parentNodeId or"",parentNodeName=aaa.parentNodeName or"root",name=aaa.name or"unnamed",id=aaa.id or
cc.timestampBaseIdGenerate(),triggerType=aaa.triggerType or d_a.ITEM_COUNT,triggerConditionType=aaa.triggerConditionType or
_aa.COUNT_GREATER,itemName=aaa.itemName or"",itemType=aaa.itemType or"item",amount=aaa.amount or 0}
...
if
self.selectNodeData.triggerType==d_a.ITEM_COUNT then if not self.selectNodeData.itemType or
...
table.insert(aaa,"Fluid name is required for Fluid Count trigger")end else
table.insert(aaa,
...
return modules["programs.SingleTurtleCrafter"](...)
