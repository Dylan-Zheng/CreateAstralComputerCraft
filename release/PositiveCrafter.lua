...
modules["programs.PositiveCrafter"] = function(...) local _c=require("libraries.basalt")
local ac=require("elements.TabView")local bc=require("utils.Logger")
...
bd:new(c_a.frame,1,1,c_a.frame:getWidth(),c_a.frame:getHeight()):init()a_a:init()parallel.waitForAll(_c.run,cd.run) end
...
modules["programs.crafter.positive.CraftingListTab"] = function(...) local db=require("libraries.basalt")
local _c=require("utils.Logger")local ac=require("elements.ItemSelectedListBox")
local bc=require("programs.common.SnapShot")local cc=require("programs.recipe.manager.TriggerView")
local dc=require("elements.MessageBox")local _d=require("elements.ConfirmMessageBox")
local ad=require("elements.ScrollableFrame")
local bd=require("programs.crafter.positive.RecipeManager")
local cd=require("programs.crafter.positive.RecipeBarrelLoader")
local function dd(b_a)if not b_a then return b_a end;local c_a=string.find(b_a,":")if c_a then return string.sub(b_a,
c_a+1)else return b_a end end
local __a=function(b_a,c_a)local d_a={}if b_a then
table.insert(d_a,{text=b_a,selected=true})end;for _aa,aaa in pairs(c_a)do if b_a~=_aa then
table.insert(d_a,{text=_aa})end end;return d_a end;local a_a={}a_a.__index=a_a
function a_a:new(b_a,c_a,d_a,_aa,aaa)local baa=setmetatable({},a_a)
baa.selectedRecipe=nil;baa._displayList={}baa.pframe=b_a;baa.selectedCrafting=nil
baa.onSelectedCallback=function()end;baa.onNewCallback=function()end
baa.getDisplayCraftingListFn=function()return{}end
baa.innerFrame=baa.pframe:addFrame():setPosition(c_a,d_a):setSize(_aa,aaa):setBackground(colors.lightGray):setForeground(colors.white)
baa.searchInput=baa.innerFrame:addInput():setPosition(2,2):setSize(18,1):setBackground(colors.gray):setForeground(colors.white):setPlaceholderColor(colors.lightGray):setPlaceholder("Search..."):onChange("text",function(_ba,aba)
baa:refreshCraftingList(aba)end)
baa.clearBtn=baa.innerFrame:addButton():setPosition(
baa.searchInput:getX()+baa.searchInput:getWidth()+1,baa.searchInput:getY()):setSize(1,1):setText("C"):setBackground(colors.gray):setForeground(colors.white):onClick(function()
baa.searchInput:setText("")baa:refreshCraftingList()end)
baa.craftingList=baa.innerFrame:addList():setPosition(2,
baa.searchInput:getY()+baa.searchInput:getHeight()+1):setSize(20,
baa.innerFrame:getHeight()- (
baa.searchInput:getY()+baa.searchInput:getHeight()+3)):setBackground(colors.gray):setForeground(colors.white):onSelect(function(_ba,aba,bba)
if

baa.selectedRecipe and baa.selectedRecipe.output==bba.recipe.output then bba.selected=false;baa.selectedRecipe=nil;baa:setDetails(nil)
baa.craftingList:render()else baa:setDetails(bba.recipe)end end)
baa.newButton=baa.innerFrame:addButton():setPosition(baa.craftingList:getX(),
baa.craftingList:getY()+baa.craftingList:getHeight()+1):setSize(7,1):setText("New"):setBackground(colors.gray):setForeground(colors.white):onClick(function()baa:setDetails(
nil)end)
baa.deleteButton=baa.innerFrame:addButton():setPosition(
baa.newButton:getX()+baa.newButton:getWidth()+1,baa.newButton:getY()):setSize(7,1):setText("Delete"):setBackground(colors.gray):setForeground(colors.white):onClick(function()
if
baa.selectedRecipe then
baa.confirmMessageBox:open("Delete Recipe","Are you sure you want to delete the selected recipe .. ".. (
baa.selectedRecipe.output or"empty").." ?",function()
bd.removeRecipe(baa.selectedRecipe)
baa.craftingList:setItems(baa:refreshCraftingList(baa.searchInput:getText()))baa:setDetails(nil)end)else
baa.messageBox:open("Error","No recipe selected to delete.")end end)
baa.detailFrame=baa.innerFrame:addFrame():setPosition(
baa.craftingList:getX()+baa.craftingList:getWidth()+1,2):setSize(
baa.innerFrame:getWidth()- (
baa.craftingList:getX()+baa.craftingList:getWidth()+1),
baa.innerFrame:getHeight()-2):setBackground(colors.gray):setForeground(colors.white)
baa.outputLabel=baa.detailFrame:addLabel():setPosition(2,2):setSize(
baa.detailFrame:getWidth()-3,1):setBackground(colors.gray):setForeground(colors.white):setText("Output: ")
baa.outputValueLabel=baa.detailFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(2,
baa.outputLabel:getY()+2):setSize(
baa.detailFrame:getWidth()-3,1):setText("empty"):setBackground(colors.lightGray):setForeground(colors.white):onClick(function()
baa.itemListBox:open(__a(
baa.selectedRecipe and baa.selectedRecipe.output or nil,bc.items),false,{confirm=function(_ba)baa.selectedRecipe=
baa.selectedRecipe or{}
baa.selectedRecipe.output=_ba[1].text;baa:setDetails(baa.selectedRecipe)end})end)
baa.inputInvLabel=baa.detailFrame:addLabel():setPosition(2,
baa.outputValueLabel:getY()+baa.outputValueLabel:getHeight()+1):setSize(
baa.detailFrame:getWidth()-3,1):setBackground(colors.gray):setForeground(colors.white):setText("Input: ")local caa={"i1","i2","i3","i4","i5","i6","i7","i8","i9"}
local daa=
baa.inputInvLabel:getY()+baa.inputInvLabel:getHeight()+1
for i=1,9 do
baa.detailFrame:addLabel():setPosition(2,daa):setBackgroundEnabled(true):setPosition(2,daa):setSize(
baa.detailFrame:getWidth()-3,1):setBackground(colors.lightGray):setForeground(colors.white):setText(
i..". ")
baa[caa[i]]=baa.detailFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(5,daa):setSize(
baa.detailFrame:getWidth()-6,1):setBackground(colors.lightGray):setForeground(colors.white):setText("empty"):onClick(function()
baa.itemListBox:open(__a(
baa.selectedRecipe and baa.selectedRecipe.inputs and
baa.selectedRecipe.inputs[i]or nil,bc.items),false,{confirm=function(_ba)baa.selectedRecipe=
baa.selectedRecipe or{}baa.selectedRecipe.inputs=
baa.selectedRecipe.inputs or{}
baa.selectedRecipe.inputs[i]=_ba[1].text;baa:setDetails(baa.selectedRecipe)end})end)
daa=baa[caa[i]]:getY()+baa[caa[i]]:getHeight()end
baa.maxInputsLabel=baa.detailFrame:addLabel():setPosition(2,daa+1):setForeground(colors.white):setText("Max Inputs: ")
baa.maxInputBox=baa.detailFrame:addInput():setPosition(
baa.maxInputsLabel:getX()+baa.maxInputsLabel:getWidth()+1,baa.maxInputsLabel:getY()):setSize(
baa.detailFrame:getWidth()- (
baa.maxInputsLabel:getX()+baa.maxInputsLabel:getWidth()+2),1):setBackground(colors.lightGray):setForeground(colors.black):setText("0"):setPattern("[0-9]")
baa.triggerLabel=baa.detailFrame:addLabel():setPosition(2,
baa.maxInputsLabel:getY()+baa.maxInputBox:getHeight()+1):setText("Trigger:"):setForeground(colors.white)
baa.editTriggerBtn=baa.detailFrame:addButton():setPosition(
baa.triggerLabel:getX()+baa.triggerLabel:getWidth()+3,baa.triggerLabel:getY()):setSize(6,1):setText("Edit"):setBackground(colors.lightGray):setForeground(colors.black):onClick(function()
baa.triggerBox:open(
baa.selectedRecipe and baa.selectedRecipe.trigger or nil,function(_ba)baa.selectedRecipe=
baa.selectedRecipe or{}baa.selectedRecipe.trigger=_ba end)end)
baa.clearTriggerBtn=baa.detailFrame:addButton():setPosition(
baa.editTriggerBtn:getX()+baa.editTriggerBtn:getWidth()+1,baa.editTriggerBtn:getY()):setSize(7,1):setText("Clear"):setBackground(colors.lightGray):setForeground(colors.black):onClick(function()if
baa.selectedRecipe then baa.selectedRecipe.trigger=nil end end)
baa.loadBtn=baa.detailFrame:addButton():setPosition(
baa.detailFrame:getWidth()-14,baa.clearTriggerBtn:getY()+
baa.clearTriggerBtn:getHeight()+1):setSize(6,1):setText("Load"):setBackground(colors.lightGray):setForeground(colors.black):onClick(function()
local _ba,aba=cd.load()if _ba then baa:setDetails(_ba)else
baa.messageBox:open("Error","Error loading recipe from barrel: ".. (aba or
"unknown error"))end end)
baa.saveBtn=baa.detailFrame:addButton():setPosition(
baa.detailFrame:getWidth()-7,baa.loadBtn:getY()):setSize(6,1):setText("Save"):setBackground(colors.green):setForeground(colors.black):onClick(function()
if
baa.selectedRecipe then
local _ba=tonumber(baa.maxInputBox:getText())or 0;if _ba<0 or _ba>64 then
baa.messageBox:open("Error","Max Inputs must be between 0 and 64.")return end
baa.selectedRecipe.maxInputs=_ba;local aba,bba=bd.addRecipe(baa.selectedRecipe)
if aba then
baa:refreshCraftingList(baa.searchInput:getText())
baa.messageBox:open("Success","Recipe saved successfully.")else
baa.messageBox:open("Error","Error saving recipe: ".. (bba or"unknown error"))end else
baa.messageBox:open("Error","No recipe selected to save.")end end)
baa.detailFrame:addLabel():setPosition(2,
baa.saveBtn:getY()+1):setText("")
ad.setScrollable(baa.detailFrame,true,colors.gray,colors.lightGray,colors.gray,colors.white)baa.itemListBox=ac:new(baa.pframe)
baa.triggerBox=cc:new(baa.pframe)baa.messageBox=dc:new(baa.pframe)
baa.confirmMessageBox=_d:new(baa.pframe)return baa end
function a_a:getDisplayList()
if bd.checkedUpdate()then local b_a=bd.getRecipes()
self._displayList={}for c_a,d_a in pairs(b_a)do
table.insert(self._displayList,{text=dd(c_a),recipe=d_a})end end;return self._displayList end
function a_a:refreshCraftingList(b_a)local c_a=self:getDisplayList()local d_a={}
if b_a and b_a~=""then
local _aa=string.lower(b_a)
for aaa,baa in ipairs(c_a)do if string.find(string.lower(baa.text),_aa,1,true)then
table.insert(d_a,baa)end end else for _aa,aaa in ipairs(c_a)do table.insert(d_a,aaa)end end;self.craftingList:setItems(d_a)end
function a_a:setDetails(b_a)
if b_a then self.selectedRecipe=b_a
self.outputValueLabel:setText(b_a.output or"empty")local c_a=b_a.inputs or{}for i=1,9 do local d_a=self["i"..i]local _aa=nil;if c_a[i]then
_aa=dd(c_a[i])end
if d_a then d_a:setText(_aa or"empty")end end
self.maxInputBox:setText(tostring(
b_a.maxInputs or 0))else self.selectedRecipe={inputs={},output=nil,trigger=nil,maxInputs=64}
self.outputValueLabel:setText("empty")
for i=1,9 do local c_a=self["i"..i]if c_a then c_a:setText("empty")end end;self.maxInputBox:setText("64")end end;function a_a:init()self:refreshCraftingList()
self:setDetails(nil)return self end;return a_a end
...
modules["programs.crafter.positive.TurtleTab"] = function(...) local ba=require("libraries.basalt")
local ca=require("utils.Logger")local da=require("elements.ScrollableFrame")
local _b=require("programs.crafter.positive.TaskDispatchMaster")
local function ab(cb)if not cb then return cb end;local db=string.find(cb,":")if db then return
string.sub(cb,db+1)else return cb end end;local bb={}bb.__index=bb
function bb:new(cb,db,_c,ac,bc)local cc=setmetatable({},bb)
cc.selectedTurtle=nil;cc.selectedTurtleId=nil;cc._displayList={}cc.pframe=cb
cc.innerFrame=cc.pframe:addFrame():setPosition(db,_c):setSize(ac,bc):setBackground(colors.lightGray):setForeground(colors.white)
cc.searchInput=cc.innerFrame:addInput():setPosition(2,2):setSize(18,1):setBackground(colors.gray):setForeground(colors.white):setPlaceholderColor(colors.lightGray):setPlaceholder("Search..."):onChange("text",function(dc,_d)
cc:refreshTurtleList(_d)end)
cc.clearBtn=cc.innerFrame:addButton():setPosition(
cc.searchInput:getX()+cc.searchInput:getWidth()+1,cc.searchInput:getY()):setSize(1,1):setText("C"):setBackground(colors.gray):setForeground(colors.white):onClick(function()
cc.searchInput:setText("")cc:refreshTurtleList()end)
cc.turtleList=cc.innerFrame:addList():setPosition(2,
cc.searchInput:getY()+cc.searchInput:getHeight()+1):setSize(20,
cc.innerFrame:getHeight()- (
cc.searchInput:getY()+cc.searchInput:getHeight()+1)):setBackground(colors.gray):setForeground(colors.white):onSelect(function(dc,_d,ad)
if
cc.selectedTurtleId and cc.selectedTurtleId==ad.turtleId then
ad.selected=false;cc.selectedTurtle=nil;cc.selectedTurtleId=nil
cc:setDetails(nil,nil)cc.turtleList:render()else
cc:setDetails(ad.turtleId,ad.turtle)end end)
cc.detailFrame=cc.innerFrame:addFrame():setPosition(
cc.turtleList:getX()+cc.turtleList:getWidth()+1,2):setSize(
cc.innerFrame:getWidth()- (
cc.turtleList:getX()+cc.turtleList:getWidth()+1),
cc.innerFrame:getHeight()-2):setBackground(colors.gray):setForeground(colors.white)
cc.turtleIdLabel=cc.detailFrame:addLabel():setPosition(2,2):setSize(
cc.detailFrame:getWidth()-3,1):setBackground(colors.gray):setForeground(colors.white):setText("Turtle ID: ")
cc.turtleIdValueLabel=cc.detailFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(2,
cc.turtleIdLabel:getY()+2):setSize(
cc.detailFrame:getWidth()-3,1):setText("None selected"):setBackground(colors.lightGray):setForeground(colors.white)
cc.nameLabel=cc.detailFrame:addLabel():setPosition(2,
cc.turtleIdValueLabel:getY()+cc.turtleIdValueLabel:getHeight()+
1):setSize(
cc.detailFrame:getWidth()-3,1):setBackground(colors.gray):setForeground(colors.white):setText("Name: ")
cc.nameValueLabel=cc.detailFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(2,
cc.nameLabel:getY()+1):setSize(
cc.detailFrame:getWidth()-3,1):setText("N/A"):setBackground(colors.lightGray):setForeground(colors.white)
cc.statusLabel=cc.detailFrame:addLabel():setPosition(2,
cc.nameValueLabel:getY()+cc.nameValueLabel:getHeight()+1):setSize(
cc.detailFrame:getWidth()-3,1):setBackground(colors.gray):setForeground(colors.white):setText("Status: ")
cc.statusValueLabel=cc.detailFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(2,
cc.statusLabel:getY()+1):setSize(
cc.detailFrame:getWidth()-3,1):setText("N/A"):setBackground(colors.lightGray):setForeground(colors.white)
cc.isOnLabel=cc.detailFrame:addLabel():setPosition(2,
cc.statusValueLabel:getY()+cc.statusValueLabel:getHeight()+1):setSize(
cc.detailFrame:getWidth()-3,1):setBackground(colors.gray):setForeground(colors.white):setText("Power Status: ")
cc.isOnValueLabel=cc.detailFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(2,
cc.isOnLabel:getY()+1):setSize(
cc.detailFrame:getWidth()-3,1):setText("N/A"):setBackground(colors.lightGray):setForeground(colors.white)
cc.recipeLabel=cc.detailFrame:addLabel():setPosition(2,
cc.isOnValueLabel:getY()+cc.isOnValueLabel:getHeight()+1):setSize(
cc.detailFrame:getWidth()-3,1):setBackground(colors.gray):setForeground(colors.white):setText("Current Recipe: ")
cc.recipeValueLabel=cc.detailFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(2,
cc.recipeLabel:getY()+1):setSize(
cc.detailFrame:getWidth()-3,1):setText("None"):setBackground(colors.lightGray):setForeground(colors.white)
cc.recipeOutputLabel=cc.detailFrame:addLabel():setPosition(2,
cc.recipeValueLabel:getY()+cc.recipeValueLabel:getHeight()+1):setSize(
cc.detailFrame:getWidth()-3,1):setBackground(colors.gray):setForeground(colors.white):setText("Recipe Output: ")
cc.recipeOutputValueLabel=cc.detailFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(2,
cc.recipeOutputLabel:getY()+1):setSize(
cc.detailFrame:getWidth()-3,1):setText("None"):setBackground(colors.lightGray):setForeground(colors.white)
cc.detailFrame:addLabel():setPosition(2,
cc.recipeOutputValueLabel:getY()+1):setText("")
da.setScrollable(cc.detailFrame,true,colors.gray,colors.lightGray,colors.gray,colors.white)return cc end
function bb:getDisplayList()local cb=_b.turtles or{}self._displayList={}
for db,_c in pairs(cb)do local ac=_c.status or
"unknown"local bc="unknown"if _c.isOn then
bc=_c.isOn()and"ON"or"OFF"end
local cc=string.format("ID: %s (%s)",tostring(db),ac)if bc~="unknown"then cc=cc.." ["..bc.."]"end
table.insert(self._displayList,{text=cc,turtleId=db,turtle=_c})end;return self._displayList end
function bb:refreshTurtleList(cb)local db=self:getDisplayList()local _c={}
if cb and cb~=""then
local ac=string.lower(cb)
for bc,cc in ipairs(db)do if string.find(string.lower(cc.text),ac,1,true)then
table.insert(_c,cc)end end else for ac,bc in ipairs(db)do table.insert(_c,bc)end end;self.turtleList:setItems(_c)end
function bb:setDetails(cb,db)
if db and cb then self.selectedTurtle=db;self.selectedTurtleId=cb
self.turtleIdValueLabel:setText(tostring(cb))local _c="N/A"if db.getName then _c=db.getName()end
self.nameValueLabel:setText(_c)local ac=db.status or"unknown"local bc=colors.lightGray;if ac=="idle"then
bc=colors.green elseif ac=="busy"then bc=colors.yellow elseif ac=="confirming"then bc=colors.orange else
bc=colors.red end
self.statusValueLabel:setText(ac):setBackground(bc)local cc="N/A"local dc=colors.lightGray
if db.isOn then local bd=db.isOn()
cc=bd and"ON"or"OFF"dc=bd and colors.green or colors.red end
self.isOnValueLabel:setText(cc):setBackground(dc)local _d=db.recipe~=nil
self.recipeValueLabel:setText(_d and"Yes"or"None")
self.recipeValueLabel:setBackground(_d and colors.green or colors.lightGray)local ad="None"if db.recipe and db.recipe.output then
ad=ab(db.recipe.output)end
self.recipeOutputValueLabel:setText(ad)else self.selectedTurtle=nil;self.selectedTurtleId=nil
self.turtleIdValueLabel:setText("None selected")self.nameValueLabel:setText("N/A")
...
self.recipeOutputValueLabel:setText("None")end end
function bb:init()self:refreshTurtleList()
self:setDetails(nil,nil)_b.addObserver(self)
ca.debug("TurtleTab registered as observer to TaskDispatchMaster")return self end;function bb:destroy()_b.removeObserver(self)
ca.debug("TurtleTab unregistered from TaskDispatchMaster")end
function bb:update()
self:refreshTurtleList(self.searchInput:getText())
if self.selectedTurtleId and _b.turtles then
local cb=_b.turtles[self.selectedTurtleId]if cb then self:setDetails(self.selectedTurtleId,cb)else self:setDetails(nil,
nil)end end end;return bb end
modules["programs.crafter.positive.TaskDispatchMaster"] = function(...) local bb=require("utils.Logger")
local cb=require("programs.common.Communicator")local db=require("wrapper.PeripheralWrapper")
local _c=require("programs.crafter.positive.RecipeManager")local ac=require("programs.common.Trigger")
local bc=require("programs.crafter.positive.RecipeInputStoreVerify")local cc={IDLE="idle",BUSY="busy",CONFIRMING="confirming"}
local dc={ASSIGN_TASK="AssignTask",RECEIVE_TASK="ReceiveTask",TASK_COMPLETED="TaskCompleted",TASK_FAILED="TaskFailed",TASK_IN_PROGRESS_HEARTBEAT="TaskInProgressHeartbeat"}local _d={}_d.openChannel=nil;_d.turtles={}_d.assignedTasks={}_d.observers={}
local ad=function()
local cd={"left","right","top","bottom","front","back"}for dd,__a in ipairs(cd)do
if peripheral.getType(__a)=="modem"then return __a end end;return nil end
local bd=function(cd,dd)if cd=="item"then return _d.storage.getItem(dd)elseif cd=="fluid"then return
_d.storage.getFluid(dd)end;return nil end;_d.getOpenChannel=function()return _d.openChannel end
_d.addObserver=function(cd)if
cd and cd.update then table.insert(_d.observers,cd)
bb.debug("Observer added to TaskDispatchMaster")end end
_d.removeObserver=function(cd)
for dd,__a in ipairs(_d.observers)do if __a==cd then
table.remove(_d.observers,dd)
bb.debug("Observer removed from TaskDispatchMaster")break end end end
_d.notifyObservers=function()for cd,dd in ipairs(_d.observers)do
if dd and dd.update then dd:update()end end end
_d.assignTask=function(cd)
if not _d.openChannel then return false,"Communication channel not initialized"end
for dd,__a in pairs(_d.turtles)do
if __a.isOn()and __a.status==cc.IDLE then
local a_a={recipe=cd,turtleId=dd}_d.openChannel.send(dc.ASSIGN_TASK,a_a,dd)
__a.status=cc.CONFIRMING;__a.recipe=cd
_d.assignedTasks[cd.output]={turtleId=dd,timestamp=os.epoch("utc")}_d.notifyObservers()return true,"Task assigned successfully"end end;return false,"No idle turtles available"end
_d.getAssignedTurtle=function(cd)local dd=_d.assignedTasks[cd]if not dd then
return nil,"No turtle assigned to this task"end;local __a=_d.turtles[dd.turtleId]if
not __a then return nil,"Assigned turtle not found"end;return __a end
_d.updateAssignedTasks=function(cd)local dd=_d.assignedTasks[cd]if dd then
dd.timestamp=os.epoch("utc")end end
_d.taskBegin=function(cd,dd)local __a=_d.turtles[cd]
if not __a then return false,"Turtle not found"end;__a.status=cc.BUSY;_d.notifyObservers()return true end
_d.clearTask=function(cd,dd)local __a=_d.turtles[cd]
if __a then __a.status=cc.IDLE;__a.recipe=nil end;_d.assignedTasks[dd]=nil;_d.notifyObservers()return true end
_d.isAssigned=function(cd)return _d.assignedTasks[cd]~=nil end
_d.initEvents=function()
if not _d.openChannel then return false,"Open channel not initialized"end
_d.openChannel.addMessageHandler(dc.RECEIVE_TASK,function(cd,dd,__a)
_d.taskBegin(__a,dd.recipeName)end)
_d.openChannel.addMessageHandler(dc.TASK_COMPLETED,function(cd,dd,__a)
_d.clearTask(__a,dd.recipeName)end)
_d.openChannel.addMessageHandler(dc.TASK_IN_PROGRESS_HEARTBEAT,function(cd,dd,__a)
local a_a=_d.getAssignedTurtle(dd.recipeName)if not a_a then local b_a=_d.turtles[__a]if b_a then b_a.reboot()end
_d.clearTask(__a,dd.recipeName)return end
_d.updateAssignedTasks(dd.recipeName)end)
_d.openChannel.addMessageHandler(dc.TASK_FAILED,function(cd,dd,__a)
_d.clearTask(__a,dd.recipeName)end)return true end
_d.checkAssignedTasks=function(cd)local dd=os.epoch("utc")for __a,a_a in pairs(_d.assignedTasks)do
if
dd-a_a.timestamp>cd then
_d.turtles[a_a.turtleId].reboot()_d.clearTask(a_a.turtleId,__a)end end end
_d.checkRecipesTrigger=function()if not _c.recipes then return end
for cd,dd in pairs(_c.recipes)do
if
dd and dd.output and dd.trigger then
if not _d.isAssigned(dd.output)then
if
ac.eval(dd.trigger,bd)and bc.verify(dd,_d.storage)then
local __a,a_a=_d.assignTask(dd)if not __a then
bb.error("Failed to assign task for recipe {}: {}",dd.output,a_a)end end end end end end
_d.init=function()local cd=ad()
if not cd then error("No modem peripheral found")end;_d.openChannel=cb.open(cd,123,"TaskDispatch","default")
_d.initEvents()
local dd=db.getAllPeripheralsNameContains("crafting_storage")local __a=next(dd)local a_a=dd[__a]if not a_a then
error("No crafting_storage peripheral found")end;_d.storage=a_a
local b_a=db.getAllPeripheralsNameContains("turtle")
for c_a,d_a in pairs(b_a)do if not d_a.isOn()then d_a.turnOn()end
d_a.status=cc.IDLE;d_a.recipe=nil;local _aa=d_a.getID()
_d.turtles[tonumber(_aa)or _aa]=d_a end;_d.notifyObservers()end
_d.run=function()_d.init()
parallel.waitForAll(function()while true do _d.checkAssignedTasks(60000)
os.sleep(10)end end,function()while true do
_d.checkRecipesTrigger()os.sleep(0.2)end end,cb.listen)end;return _d end
...
modules["programs.crafter.positive.RecipeBarrelLoader"] = function(...) local ba=require("utils.Logger")
local ca=require("wrapper.PeripheralWrapper")local da=nil;local _b={}local ab={1,2,3,10,11,12,19,20,21}local bb=4
_b.load=function()if not da then
local _c=ca.getAllPeripheralsNameContains("barrel")local ac=next(_c)da=_c[ac]
if not da then return false,"No barrel found"end end
local cb={output=nil,inputs={},maxInputs=64}
for _c,ac in pairs(ab)do local bc=da.getItemDetail(ac)if bc and bc.name then
cb.inputs[_c]=bc.name end end;local db=da.getItemDetail(bb)cb.output=db and db.name;return cb end;return _b end
...
return modules["programs.PositiveCrafter"](...)
