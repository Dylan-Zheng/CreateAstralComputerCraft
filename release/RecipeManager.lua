...
modules["programs.RecipeManager"] = function(...) local ad=require("libraries.basalt")
local bd=require("elements.TabView")
local cd=require("programs.recipe.manager.DepotRecipeTab")local dd=require("programs.recipe.manager.StoreManager")
local __a=require("programs.common.SnapShot")local a_a=require("utils.Logger")
local b_a=require("programs.recipe.manager.SettingTab")local c_a=require("programs.common.Communicator")
local d_a=require("programs.recipe.manager.BasinRecipeTab")
local _aa=require("programs.recipe.manager.BeltRecipeTab")
local aaa=require("programs.recipe.manager.CommonRecipeTab")local baa=true;ad.LOGGER.setEnabled(baa)
ad.LOGGER.setLogToFile(true)
if baa then
a_a.addPrintFunction(function(aca,bca,cca,dca)
dca=string.format("[%s:%d] %s",bca,cca,dca)
if aca==a_a.levels.DEBUG then ad.LOGGER.debug(dca)elseif aca==
a_a.levels.INFO then ad.LOGGER.info(dca)elseif aca==a_a.levels.WARN then
ad.LOGGER.warn(dca)elseif aca==a_a.levels.ERROR then ad.LOGGER.error(dca)end end)end;dd.init()__a.takeSnapShot()c_a.loadSettings()
local caa=c_a.getOpenChannels()[1]
caa.addMessageHandler("getRecipesReq",function(aca,bca,cca)local dca=dd.getAllRecipesByType(bca)
caa.send("getRecipesRes",dca,cca)end)local daa=ad.getMainFrame()
local _ba=bd:new(daa:addFrame(),1,1,daa:getWidth(),daa:getHeight())local aba=_ba:createTab("Depot")
local bba=_ba:createTab("Basin")local cba=_ba:createTab("Belt")
local dba=_ba:createTab("Common")local _ca=_ba:createTab("Settings")
cd:new(aba.frame):init()d_a:new(bba.frame):init()
_aa:new(cba.frame):init()aaa:new(dba.frame):init()
b_a:new(_ca.frame):init()_ba:init()
parallel.waitForAll(ad.run,c_a.listen) end
