...
modules["programs.CreateCrafter"] = function(...) local db=require("libraries.basalt")
...
_d:new(__a.frame):init()local a_a=dd:createTab("Settings")
dc:new(a_a.frame):init()dd:init()parallel.waitForAll(ad.run,db.run) end
...
modules["programs.crafter.create.SettingTab"] = function(...) local ca=require("utils.Logger")
local da=require("utils.OSUtils")local _b=require("programs.crafter.create.Manager")
local ab=require("wrapper.PeripheralWrapper")local bb=require("elements.ItemSelectedListBox")local cb=colors
local db={}db.__index=db
function db:new(_c)local ac=setmetatable({},db)ac.pframe=_c
ac.innerFrame=ac.pframe:addFrame():setPosition(1,1):setSize(ac.pframe:getWidth(),ac.pframe:getHeight()):setBackground(cb.lightGray):setForeground(cb.white)
ac.recipesChestSettingFrame=ac.innerFrame:addFrame():setPosition(2,2):setSize(
ac.innerFrame:getWidth()-2,6):setBackground(cb.gray):setForeground(cb.white)
ac.recipesChestTitleLabel=ac.recipesChestSettingFrame:addLabel():setText("Recipes Chest Setting"):setPosition(2,2):setBackground(cb.gray):setForeground(cb.white)
ac.chestLabel=ac.recipesChestSettingFrame:addLabel():setText("Chest:"):setPosition(2,4):setBackground(cb.gray):setForeground(cb.white)
ac.chestValueLabel=ac.recipesChestSettingFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(
ac.chestLabel:getX()+ac.chestLabel:getWidth()+1,ac.chestLabel:getY()):setSize(
ac.recipesChestSettingFrame:getWidth()- (
ac.chestLabel:getX()+ac.chestLabel:getWidth()+1),1):setText("..."):setBackground(cb.lightGray):setForeground(cb.white):onClick(function()
ac:openChestSelector()end)
ac.widthLabel=ac.recipesChestSettingFrame:addLabel():setText("Width:"):setPosition(2,
ac.chestLabel:getY()+ac.chestLabel:getHeight()):setBackground(cb.gray):setForeground(cb.white)
ac.widthInput=ac.recipesChestSettingFrame:addInput():setPosition(
ac.widthLabel:getX()+ac.widthLabel:getWidth()+1,ac.widthLabel:getY()):setSize(4,1):setBackground(cb.black):setForeground(cb.white):setPattern("^[0-9]*$"):setText("0")
ac.heightLabel=ac.recipesChestSettingFrame:addLabel():setText("Height:"):setPosition(
ac.widthInput:getX()+ac.widthInput:getWidth()+2,ac.widthLabel:getY()):setBackground(cb.gray):setForeground(cb.white)
ac.heightInput=ac.recipesChestSettingFrame:addInput():setPosition(
ac.heightLabel:getX()+ac.heightLabel:getWidth()+1,ac.heightLabel:getY()):setSize(4,1):setBackground(cb.black):setForeground(cb.white):setPattern("^[0-9]*$"):setText("0")
ac.crafterSettingFrame=ac.innerFrame:addFrame():setPosition(2,

ac.recipesChestSettingFrame:getY()+ac.recipesChestSettingFrame:getHeight()+1):setSize(
ac.innerFrame:getWidth()-2,5):setBackground(cb.gray):setForeground(cb.white)
ac.crafterTitleLabel=ac.crafterSettingFrame:addLabel():setText("Crafter Setting"):setPosition(2,2):setBackground(cb.gray):setForeground(cb.white)
ac.crafterWidthLabel=ac.crafterSettingFrame:addLabel():setText("Width:"):setPosition(2,4):setBackground(cb.gray):setForeground(cb.white)
ac.crafterWidthInput=ac.crafterSettingFrame:addInput():setPosition(
ac.crafterWidthLabel:getX()+ac.crafterWidthLabel:getWidth()+1,ac.crafterWidthLabel:getY()):setSize(4,1):setBackground(cb.black):setForeground(cb.white):setPattern("^[0-9]*$"):setText("0")
ac.crafterHeightLabel=ac.crafterSettingFrame:addLabel():setText("Height:"):setPosition(
ac.crafterWidthInput:getX()+ac.crafterWidthInput:getWidth()+2,ac.crafterWidthLabel:getY()):setBackground(cb.gray):setForeground(cb.white)
ac.crafterHeightInput=ac.crafterSettingFrame:addInput():setPosition(
ac.crafterHeightLabel:getX()+ac.crafterHeightLabel:getWidth()+1,ac.crafterHeightLabel:getY()):setSize(4,1):setBackground(cb.black):setForeground(cb.white):setPattern("^[0-9]*$"):setText("0")
ac.saveButton=ac.innerFrame:addButton():setText("Save"):setPosition(2,

ac.crafterSettingFrame:getY()+ac.crafterSettingFrame:getHeight()+1):setSize(10,3):setBackground(cb.green):setForeground(cb.white)ac.itemListBox=bb:new(ac.pframe)return ac end
function db:init()local _c=_b.getSettings()
ca.debug("Loaded settings: {}",_c)
self.widthInput:setText(tostring(
_c.recipesChestSize and _c.recipesChestSize.width or 0))
self.heightInput:setText(tostring(
_c.recipesChestSize and _c.recipesChestSize.height or 0))
self.crafterWidthInput:setText(tostring(
_c.crafterSize and _c.crafterSize.width or 0))
self.crafterHeightInput:setText(tostring(
_c.crafterSize and _c.crafterSize.height or 0))if _c.recipesChest and _c.recipesChest~=""then
self.chestValueLabel:setText(_c.recipesChest)end
self.saveButton:onClick(function()local ac=
tonumber(self.widthInput:getText())or 0;local bc=
tonumber(self.heightInput:getText())or 0;local cc=
tonumber(self.crafterWidthInput:getText())or 0;local dc=
tonumber(self.crafterHeightInput:getText())or 0
local _d=self.chestValueLabel:getText()
_b.setSettings({crafterSize={width=cc,height=dc},recipesChestSize={width=ac,height=bc},recipesChest=_d~="..."and _d or nil})os.reboot()end)end
function db:openChestSelector()ab.reloadAll()
local _c=ab.getAllPeripheralsNameContains("chest")local ac={}local bc=self.chestValueLabel:getText()for cc,dc in pairs(_c)do
table.insert(ac,{text=cc,selected=(
cc==bc and bc~="...")})end
self.itemListBox:open(ac,false,{confirm=function(cc)
if
cc and#cc>0 then self.chestValueLabel:setText(cc[1].text)end end})end;return db end
modules["programs.crafter.create.RecipesTab"] = function(...) local ac=require("libraries.basalt")
local bc=require("utils.Logger")local cc=require("utils.OSUtils")
local dc=require("programs.crafter.create.Manager")local _d=require("elements.ItemSelectedListBox")
local ad=require("programs.common.SnapShot")local bd=require("programs.recipe.manager.TriggerView")
local cd=require("elements.MessageBox")local dd=require("elements.ConfirmMessageBox")
local __a=require("elements.ScrollableFrame")local a_a=require("wrapper.PeripheralWrapper")local b_a=colors
local function c_a(aaa)if not aaa then return
aaa end;local baa=string.find(aaa,":")if baa then return
string.sub(aaa,baa+1)else return aaa end end
local d_a=function(aaa,baa)local caa={}if aaa then
table.insert(caa,{text=aaa,selected=true})end;for daa,_ba in pairs(baa)do if aaa~=daa then
table.insert(caa,{text=daa})end end;return caa end;local _aa={}_aa.__index=_aa
function _aa:new(aaa)local baa=setmetatable({},_aa)
baa.selectedRecipe=nil;baa._displayList={}baa.pframe=aaa
baa.innerFrame=baa.pframe:addFrame():setPosition(1,1):setSize(baa.pframe:getWidth(),baa.pframe:getHeight()):setBackground(b_a.lightGray):setForeground(b_a.white)
baa.searchInput=baa.innerFrame:addInput():setPosition(2,2):setSize(18,1):setBackground(b_a.gray):setForeground(b_a.white):setPlaceholderColor(b_a.lightGray):setPlaceholder("Search..."):onChange("text",function(caa,daa)
baa:refreshRecipeList(daa)end)
baa.clearBtn=baa.innerFrame:addButton():setPosition(
baa.searchInput:getX()+baa.searchInput:getWidth()+1,baa.searchInput:getY()):setSize(1,1):setText("C"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
baa.searchInput:setText("")baa:refreshRecipeList()end)
baa.recipeList=baa.innerFrame:addList():setPosition(2,
baa.searchInput:getY()+baa.searchInput:getHeight()+1):setSize(20,
baa.innerFrame:getHeight()- (
baa.searchInput:getY()+baa.searchInput:getHeight()+3)):setBackground(b_a.gray):setForeground(b_a.white):onSelect(function(caa,daa,_ba)
if

baa.selectedRecipe and baa.selectedRecipe.id==_ba.recipe.id then _ba.selected=false;baa.selectedRecipe=nil;baa:setDetails(nil)
baa.recipeList:render()else baa:setDetails(_ba.recipe)end end)
baa.newButton=baa.innerFrame:addButton():setPosition(baa.recipeList:getX(),
baa.recipeList:getY()+baa.recipeList:getHeight()+1):setSize(7,1):setText("New"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()baa:setDetails(
nil)end)
baa.deleteButton=baa.innerFrame:addButton():setPosition(
baa.newButton:getX()+baa.newButton:getWidth()+1,baa.newButton:getY()):setSize(7,1):setText("Delete"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
if
baa.selectedRecipe then
baa.confirmMessageBox:open("Delete Recipe","Are you sure you want to delete the selected recipe with ID ".. (
baa.selectedRecipe.id or"unknown").." ?",function()
dc.deleteRecipeById(baa.selectedRecipe.id)
baa.recipeList:setItems(baa:refreshRecipeList(baa.searchInput:getText()))baa:setDetails(nil)end)else
baa.messageBox:open("Error","No recipe selected to delete.")end end)
baa.detailFrame=baa.innerFrame:addFrame():setPosition(
baa.recipeList:getX()+baa.recipeList:getWidth()+1,2):setSize(
baa.innerFrame:getWidth()- (
baa.recipeList:getX()+baa.recipeList:getWidth()+1),
baa.innerFrame:getHeight()-2):setBackground(b_a.gray):setForeground(b_a.white)
baa.outputLabel=baa.detailFrame:addLabel():setPosition(2,2):setSize(
baa.detailFrame:getWidth()-3,1):setBackground(b_a.gray):setForeground(b_a.white):setText("Output: ")
...
baa.detailFrame:getWidth()-3,1):setText("empty"):setBackground(b_a.lightGray):setForeground(b_a.white):onClick(function()local caa=
nil
if baa.selectedRecipe and baa.selectedRecipe.output then
if

type(baa.selectedRecipe.output)=="table"and baa.selectedRecipe.output.name then caa=baa.selectedRecipe.output.name elseif
type(baa.selectedRecipe.output)=="string"then caa=baa.selectedRecipe.output end end
baa.itemListBox:open(d_a(caa,ad.items),false,{confirm=function(daa)
baa.selectedRecipe=baa.selectedRecipe or{}baa.selectedRecipe.output={name=daa[1].text}
baa:setDetails(baa.selectedRecipe)end})end)
baa.triggerLabel=baa.detailFrame:addLabel():setPosition(2,
baa.outputValueLabel:getY()+baa.outputValueLabel:getHeight()+1):setText("Trigger:"):setForeground(b_a.white)
baa.editTriggerBtn=baa.detailFrame:addButton():setPosition(
baa.triggerLabel:getX()+baa.triggerLabel:getWidth()+3,baa.triggerLabel:getY()):setSize(6,1):setText("Edit"):setBackground(b_a.lightGray):setForeground(b_a.black):onClick(function()
baa.triggerBox:open(
baa.selectedRecipe and baa.selectedRecipe.trigger or nil,function(caa)baa.selectedRecipe=
baa.selectedRecipe or{}baa.selectedRecipe.trigger=caa end)end)
baa.clearTriggerBtn=baa.detailFrame:addButton():setPosition(
baa.editTriggerBtn:getX()+baa.editTriggerBtn:getWidth()+1,baa.editTriggerBtn:getY()):setSize(7,1):setText("Clear"):setBackground(b_a.lightGray):setForeground(b_a.black):onClick(function()if
baa.selectedRecipe then baa.selectedRecipe.trigger=nil end end)
baa.isDisabledCheckBox=baa.detailFrame:addCheckbox():setSize(1,1):setBackground(b_a.gray):setForeground(b_a.white):setText("[ ] Disabled"):setCheckedText("[x] Disabled"):setChecked(false):setPosition(2,
baa.clearTriggerBtn:getY()+baa.clearTriggerBtn:getHeight()+1)
baa.loadBtn=baa.detailFrame:addButton():setPosition(
baa.detailFrame:getWidth()-14,baa.isDisabledCheckBox:getY()+
baa.isDisabledCheckBox:getHeight()+1):setSize(6,1):setText("Load"):setBackground(b_a.lightGray):setForeground(b_a.black):onClick(function()
local caa=dc.getSettings().recipesChest;if not caa or caa==""then
baa.messageBox:open("Error","No recipes chest configured in settings.")return end
local daa=a_a.getByName(caa)if not daa then
baa.messageBox:open("Error","Could not find peripheral: "..caa)return end
local _ba=dc.readRecipesFromChest(daa)
if _ba then baa:setDetails(_ba)
baa.messageBox:open("Success","Recipe loaded from chest successfully.")else
baa.messageBox:open("Error","Failed to load recipe from chest. Check chest contents and settings.")end end)
baa.saveBtn=baa.detailFrame:addButton():setPosition(
baa.detailFrame:getWidth()-7,baa.loadBtn:getY()):setSize(6,1):setText("Save"):setBackground(b_a.green):setForeground(b_a.black):onClick(function()
if
baa.selectedRecipe then local caa,daa=baa:saveRecipe()
if caa then
baa:refreshRecipeList(baa.searchInput:getText())
baa.messageBox:open("Success","Recipe saved successfully.")else
baa.messageBox:open("Error","Error saving recipe: ".. (daa or"unknown error"))end else
baa.messageBox:open("Error","No recipe selected to save.")end end)
baa.inputLabel=baa.detailFrame:addLabel():setPosition(2,
baa.saveBtn:getY()+baa.saveBtn:getHeight()+1):setSize(
baa.detailFrame:getWidth()-3,1):setBackground(b_a.gray):setForeground(b_a.white):setText("Input: ")baa.inputLabels={}baa.inputSlots={}
baa.itemListBox=_d:new(baa.pframe)baa.triggerBox=bd:new(baa.pframe)
baa.messageBox=cd:new(baa.pframe)baa.confirmMessageBox=dd:new(baa.pframe)
baa.emptyLabel=baa.detailFrame:addLabel():setText("")baa:createInputSlots()
__a.setScrollable(baa.detailFrame,true,b_a.gray,b_a.lightGray,b_a.gray,b_a.white)return baa end
function _aa:createInputSlots()local aaa=dc.getSettings()local baa=3;local caa=3;if aaa and aaa.crafterSize then baa=
aaa.crafterSize.width or 3
caa=aaa.crafterSize.height or 3 end;local daa=baa*caa;for i=1,#self.inputSlots
do if self.inputSlots[i]then
self.inputSlots[i]:remove()end end
self.inputSlots={}local _ba=
self.inputLabel:getY()+self.inputLabel:getHeight()+1
for i=1,daa do
local aba=self.detailFrame:addLabel():setPosition(2,_ba):setBackgroundEnabled(true):setSize(
self.detailFrame:getWidth()-3,1):setBackground(b_a.lightGray):setForeground(b_a.white):setText(
i..". ")
local bba=self.detailFrame:addLabel():setAutoSize(false):setBackgroundEnabled(true):setPosition(5,_ba):setSize(
self.detailFrame:getWidth()-6,1):setBackground(b_a.lightGray):setForeground(b_a.white):setText("empty"):onClick(function()
self.itemListBox:open(d_a(
self.selectedRecipe and self.selectedRecipe.input and
self.selectedRecipe.input[i]or nil,ad.items),false,{confirm=function(cba)self.selectedRecipe=
self.selectedRecipe or{}self.selectedRecipe.input=
self.selectedRecipe.input or{}
self.selectedRecipe.input[i]=cba[1].text;self:setDetails(self.selectedRecipe)end})end)self.inputSlots[i]=bba
_ba=bba:getY()+bba:getHeight()end;self.emptyLabel:setPosition(2,_ba+1)end
function _aa:getDisplayList()local aaa=dc.getRecipes()self._displayList={}
if aaa then
for baa,caa in ipairs(aaa)do local daa=
"Recipe ".. (caa.id or"unknown")
if caa.output then
if
type(caa.output)=="table"and caa.output.name then
daa=c_a(caa.output.name)elseif type(caa.output)=="string"then daa=c_a(caa.output)end end
table.insert(self._displayList,{text=daa,recipe=caa})end end;return self._displayList end
function _aa:refreshRecipeList(aaa)local baa=self:getDisplayList()local caa={}
if aaa and aaa~=""then
local daa=string.lower(aaa)
for _ba,aba in ipairs(baa)do if string.find(string.lower(aba.text),daa,1,true)then
table.insert(caa,aba)end end else for daa,_ba in ipairs(baa)do table.insert(caa,_ba)end end;self.recipeList:setItems(caa)return caa end
function _aa:setDetails(aaa)
if aaa then self.selectedRecipe=aaa;local baa="empty"
if aaa.output then
if
type(aaa.output)=="table"and aaa.output.name then
baa=c_a(aaa.output.name)elseif type(aaa.output)=="string"then baa=c_a(aaa.output)end end;self.outputValueLabel:setText(baa)
local caa=aaa.input or{}for i=1,#self.inputSlots do local daa=self.inputSlots[i]local _ba=nil;if caa[i]then
_ba=c_a(caa[i])end
if daa then daa:setText(_ba or"empty")end end;self.isDisabledCheckBox:setChecked(
aaa.isDisabled or false)else self.selectedRecipe={input={},output=
nil,trigger=nil,isDisabled=false}
self.outputValueLabel:setText("empty")for i=1,#self.inputSlots do local baa=self.inputSlots[i]if baa then
baa:setText("empty")end end
self.isDisabledCheckBox:setChecked(false)end end
function _aa:saveRecipe()
if not self.selectedRecipe then return false,"No recipe to save"end
if not self.selectedRecipe.output then return false,"Output must be specified"end
self.selectedRecipe.isDisabled=self.isDisabledCheckBox:getChecked()dc.updateRecipe(self.selectedRecipe)return true end;function _aa:init()self:refreshRecipeList()self:setDetails(nil)return
self end;return _aa end
modules["programs.crafter.create.Executor"] = function(...) local ac=require("programs.common.Trigger")
local bc=require("wrapper.PeripheralWrapper")local cc=require("programs.crafter.create.Manager")
local dc=require("utils.Logger")local _d={}
local ad=bc.getAllPeripheralsNameContains("1k_crafting_storage")local bd=ad[next(ad)]
local cd=bc.getAllPeripheralsNameContains("quad_drawer")local dd=cd[next(cd)]
local __a=bc.getAllPeripheralsNameContains("redrouter")local a_a=__a[next(__a)]
local b_a=(function()
local aaa=bc.getAllPeripheralsNameContains("create:mechanical_crafter")local baa={}for caa,daa in pairs(aaa)do table.insert(baa,daa)end
table.sort(baa,function(caa,daa)
local _ba=tonumber(string.match(caa.getName(),"(%d+)$"))
local aba=tonumber(string.match(daa.getName(),"(%d+)$"))return _ba<aba end)return baa end)()
local c_a=function(aaa)
for baa,caa in pairs(aaa)do local daa=bd.getItem(baa)if not daa or daa.count<caa then
return false end end;return true end
local d_a=function(aaa,baa)
if aaa=="item"then return bd.getItem(baa)elseif aaa=="fluid"then return bd.getFluid(baa)end;return nil end
local _aa=function(aaa)local baa=0
while true do local caa=dd.items()if caa~=nil and#caa>0 then caa[1].name=aaa
return true end;os.sleep(0.5)baa=baa+0.5;if baa>=30 then
return false end end end
_d.runRecipe=function(aaa)if aaa.isDisabled then return false,"Recipe is disabled"end
if
ac.eval(aaa.trigger,d_a)==false then return false,"Trigger condition not met"end;if not c_a(aaa.inputItemCount)then
return false,"Insufficient input items in storage"end
for baa,caa in ipairs(aaa.input)do
local daa=b_a[baa]local _ba=daa.list()if#_ba~=0 then
bd.transferItemFrom(daa,_ba[1].name,_ba[1].count)end;if caa~=false then
bd.transferItemTo(daa,caa,1)end end;a_a.setOutputSignals(true)if
not _aa(aaa.output.name)then a_a.setOutputSignals(false)aaa.isDisabled=true
return false,"Crafting timed out"end
bd.transferItemFrom(dd,aaa.output.name,64)a_a.setOutputSignals(false)return true end
_d.run=function()
while true do local aaa=1;local baa=cc.getRecipes()
for caa,daa in ipairs(baa)do
local _ba,aba=_d.runRecipe(daa)if not _ba then
dc.error("Failed to run recipe ID: {} Error: {}",daa.id,aba)else aaa=0.1
dc.info("Successfully ran recipe ID: {}",daa.id)end end;os.sleep(aaa)end end;return _d end
...
modules["programs.crafter.create.Manager"] = function(...) local aa=require("utils.OSUtils")
local ba=require("utils.Logger")
local ca={settings=nil,recipes={},shadowRecipes=nil,shouldUpdateShadowRecipes=false}
ca.getSettings=function()
if not ca.settings then
ca.settings=aa.loadTable("crafterSettings")or{crafterSize={width=0,height=0},recipesChestSize={width=0,height=0},recipesChest=
nil}end;return ca.settings end
ca.setSettings=function(ab)ca.settings=ab
aa.saveTable("crafterSettings",ca.settings)end;local da=function(ab,bb,cb)return(bb-1)*cb+ab end
ca.readRecipesFromChest=function(ab)
ba.debug("Reading recipes from chest: {}",ab.getName())local bb=ca.getSettings()local cb=bb.recipesChestSize.width
local db=bb.recipesChestSize.height;local _c=bb.crafterSize.height;local ac=bb.crafterSize.width;if
cb<=0 or db<=0 then
ba.error("Invalid recipes chest dimensions: width={}, height={}",cb,db)return nil end;if
ac<=0 or _c<=0 then
ba.error("Invalid crafter dimensions: width={}, height={}",ac,_c)return nil end
local bc={input={},output=nil}ba.debug("Crafter size: {}x{}",ac,_c)
for y=1,_c do
for x=1,ac do local dc=da(x,y,cb)
ba.debug("Checking slot {}",dc)local _d=ab.getItemDetail(dc)
if _d then
ba.debug("Found item in recipes chest: {} at slot {}",_d.name,dc)table.insert(bc.input,_d.name)else
ba.debug("No item in slot {}",dc)table.insert(bc.input,false)end end end;local cc=ab.getItemDetail(cb*db)
if cc then bc.output=cc.name end
ba.debug("Output item in recipes chest: {}",bc.output and bc.output.name or"nil")return bc end
ca._saveRecipes=function()ca.shouldUpdateShadowRecipes=true
aa.saveTable("crafterRecipes",ca.recipes)end
ca.loadRecipes=function()
ca.recipes=aa.loadTable("crafterRecipes")or{}return ca.recipes end
local _b=function(ab)
for bb,cb in ipairs(ca.recipes)do if cb.id==ab then return cb,bb end end;return nil end
ca.updateRecipe=function(ab)
if ab.id==nil then ab.id=aa.timestampBaseIdGenerate()
table.insert(ca.recipes,ab)else local bb,cb=_b(ab.id)if bb and cb then ca.recipes[cb]=ab else
table.insert(ca.recipes,ab)end end;ba.debug("Updating recipe ID: {}",ab.id)
ab.inputItemCount={}
for bb,cb in ipairs(ab.input)do
ba.debug("Processing input item: {}",cb)if cb then
ab.inputItemCount[cb]=(ab.inputItemCount[cb]or 0)+1 end end;ca._saveRecipes()end
ca.deleteRecipeById=function(ab)local bb,cb=_b(ab)if cb then table.remove(ca.recipes,cb)
ca._saveRecipes()end end
ca.getRecipes=function()
if ca.shouldUpdateShadowRecipes or ca.shadowRecipes==nil then
ca.shouldUpdateShadowRecipes=false
ca.shadowRecipes=textutils.unserialize(textutils.serialize(ca.recipes))end;return ca.shadowRecipes end;ca.loadRecipes()ca.getSettings()return ca end
...
return modules["programs.CreateCrafter"](...)
