...
modules["programs.SimpleTurtleControl"] = function() local dd=require("libraries.basalt")
local __a=require("elements.LogBox")local a_a=require("utils.Logger")local b_a=colors;local c_a=turtle;local d_a=parallel
local _aa=dd.getMainFrame():setBackground(b_a.white)
local aaa=_aa:addFrame():setPosition(1,1):setSize(_aa:getWidth(),9):setBackground(b_a.black):setForeground(b_a.white)local baa={}
for i=1,16 do local bda=( (i-1)%4)*5 +2;local cda=
math.floor((i-1)/4)*2 +2;local dda=tostring(i)
if i<10 then dda="0"..dda end
baa[i]=aaa:addButton():setPosition(bda,cda):setSize(4,1):setText(dda):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
local __b=c_a.getSelectedSlot()baa[__b]:setBackground(b_a.gray)
c_a.select(i)baa[i]:setBackground(b_a.green)end)end
local caa=aaa:addButton():setPosition(22,2):setSize(8,1):setText("Refuel"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
local bda=c_a.getSelectedSlot()local cda,dda=c_a.refuel()
if cda then
a_a.info("Refueled from slot {}, current fuel: {}",bda,c_a.getFuelLevel())else
a_a.error("Refuel failed from slot {}: {}",bda,dda or"unknown error")end end)
local daa=aaa:addButton():setPosition(caa:getX()+caa:getWidth()+1,caa:getY()):setSize(4,1):setText("Drop"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
local bda=c_a.getSelectedSlot()local cda,dda=c_a.drop()if cda then
a_a.info("Dropped items from slot {}.",bda)else
a_a.error("Drop failed from slot {}: {}",bda,dda or"unknown error")end end)
local _ba=aaa:addButton():setPosition(22,4):setSize(4,1):setText("dF"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
local bda,cda=c_a.dig()if bda then a_a.info("Dug block in front.")else
a_a.error("Dig failed: {}",cda or"unknown error")end end)
local aba=aaa:addButton():setPosition(_ba:getX()+_ba:getWidth()+1,_ba:getY()):setSize(4,1):setText("dU"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
local bda,cda=c_a.digUp()if bda then a_a.info("Dug block above.")else
a_a.error("Dig Up failed: {}",cda or"unknown error")end end)
local bba=aaa:addButton():setPosition(aba:getX()+aba:getWidth()+1,aba:getY()):setSize(4,1):setText("dD"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
local bda,cda=c_a.digDown()if bda then a_a.info("Dug block below.")else
a_a.error("Dig Down failed: {}",cda or"unknown error")end end)
local cba=aaa:addButton():setPosition(22,6):setSize(4,1):setText("pF"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
local bda,cda=c_a.place()
if bda then a_a.info("Placed block in front.")else a_a.error("Place failed: {}",cda or
"unknown error")end end)
local dba=aaa:addButton():setPosition(cba:getX()+cba:getWidth()+1,cba:getY()):setSize(4,1):setText("pU"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
local bda,cda=c_a.placeUp()if bda then a_a.info("Placed block above.")else
a_a.error("Place Up failed: {}",cda or"unknown error")end end)
local _ca=aaa:addButton():setPosition(dba:getX()+dba:getWidth()+1,dba:getY()):setSize(4,1):setText("pD"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
local bda,cda=c_a.placeDown()if bda then a_a.info("Placed block below.")else
a_a.error("Place Down failed: {}",cda or"unknown error")end end)
local aca=aaa:addButton():setPosition(22,8):setSize(4,1):setText("sF"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
local bda,cda=c_a.suck()
if bda then a_a.info("Sucked item from front.")else a_a.error("Suck failed: {}",cda or
"unknown error")end end)
local bca=aaa:addButton():setPosition(aca:getX()+aca:getWidth()+1,aca:getY()):setSize(4,1):setText("sU"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
local bda,cda=c_a.suckUp()
if bda then a_a.info("Sucked item from above.")else a_a.error("Suck Up failed: {}",cda or
"unknown error")end end)
local cca=aaa:addButton():setPosition(bca:getX()+bca:getWidth()+1,bca:getY()):setSize(4,1):setText("sD"):setBackground(b_a.gray):setForeground(b_a.white):onClick(function()
local bda,cda=c_a.suckDown()
if bda then a_a.info("Sucked item from below.")else a_a.error("Suck Down failed: {}",cda or
"unknown error")end end)
local dca=_aa:addFrame():setPosition(1,aaa:getY()+aaa:getHeight()):setSize(_aa:getWidth(),
_aa:getHeight()-9):setBackground(b_a.lightGray):setForeground(b_a.white)
local _da=__a:new(dca,2,1,dca:getWidth()-2,dca:getHeight(),b_a.white,b_a.gray)
a_a.addPrintFunction(function(bda,cda,dda,__b)_da:addLog(__b)end)
local ada=function()
while true do local bda,cda=os.pullEvent("key")local dda=""local __b=false
if cda==87 then
c_a.forward()dda="Forward"__b=true elseif cda==83 then c_a.back()dda="Back"__b=true elseif cda==65 then
c_a.turnLeft()dda="Turn Left"__b=true elseif cda==68 then c_a.turnRight()dda="Turn Right"__b=true elseif
cda==81 then c_a.up()dda="Up"__b=true elseif cda==69 then c_a.down()dda="Down"__b=true end
if __b then local a_b=c_a.getFuelLevel()local b_b=c_a.getFuelLimit()
a_a.info("Turtle {}, Fuel: {}/{}",dda,a_b,
b_b=="unlimited"and"unlimited"or tostring(b_b))end end end;d_a.waitForAll(dd.run,ada) end
...
return modules["programs.SimpleTurtleControl"]()