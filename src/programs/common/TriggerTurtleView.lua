...
function TriggerView:new(pframe)
    ...
    -- Adjusted for turtle screen - smaller proportions
    local leftWidth = math.floor(frameWidth * 1 / 3) - 1 -- 1/3 宽度，减去边距 (smaller for turtle)
    local rightWidth = frameWidth - leftWidth - 2        -- 2/3 宽度，减去边距

    instance.conditionTree = instance.frame:addTree()
        :setPosition(2, 2)
        :setSize(leftWidth - 1, frameHeight - 4) -- Reduced height for turtle screen
    ...
    instance.nodeDetailsFrame = instance.frame:addFrame()
        :setPosition(leftWidth + 2, 2) -- Reduced spacing for turtle
    ...
    instance.displayedParentNodelabel = instance.nodeDetailsFrame:addLabel()
        ...
        :setAutoSize(false)
        :setSize(rightWidth - instance.parentNodeLabel:getWidth() - 6, 1) -- Reduced for turtle
        :setBackground(colors.lightGray)
        :setForeground(colors.white)
        :setText("root")

    instance.changedParentNodeBtn = instance.nodeDetailsFrame:addButton()
        :setPosition(instance.nodeDetailsFrame:getWidth() - 3, 2) -- Smaller button for turtle
        :setSize(3, 1)                                            -- Reduced size
        :setBackground(colors.lightGray)
        :setForeground(colors.white)
        :setText("...")
        :onClick(function()
    ...
    instance.selectedItemLabel = instance.nodeDetailsFrame:addLabel()
        ...
        :setAutoSize(false)
        :setSize(instance.nodeDetailsFrame:getWidth() - instance.itemLabel:getWidth() - 6, 1) -- Reduced for turtle
        :setBackground(colors.black)
        :setForeground(colors.white)
        :setText("")

    instance.selectItemBtn = instance.nodeDetailsFrame:addButton()
        :setPosition(instance.nodeDetailsFrame:getWidth() - 3, instance.itemLabel:getY()) -- Smaller button
        :setSize(3, 1)                                                                    -- Reduced size
        :setBackground(colors.lightGray)
        :setForeground(colors.white)
        :setText("...")
    ...
    instance.amountInput = instance.nodeDetailsFrame:addInput()
        ...
        :setSize(8, 1) -- Reduced from 12 to 8 for turtle
        :setBackground(colors.lightGray)
        :setForeground(colors.white)
        :setText("0")
        :setPlaceholder("0")
    ...
    instance.modifyNodeBtn = instance.nodeDetailsFrame:addButton()
        :setPosition(instance.nodeDetailsFrame:getWidth() - 18, instance.amountInput:getY() + 2) -- Adjusted for turtle
        :setSize(5, 1)                                                                           -- Reduced from 8 to 5
        :setBackground(colors.blue)
        :setForeground(colors.white)
        :setText("Mod")    -- Shortened text
    ...
    instance.addNodeBtn = instance.nodeDetailsFrame:addButton()
        :setPosition(instance.nodeDetailsFrame:getWidth() - 12, instance.amountInput:getY() + 2) -- Adjusted for turtle
        :setSize(4, 1)                                                                           -- Reduced from 6 to 4
    ...
    instance.nodeDetailsFrame:addLabel()
        :setPosition(2, instance.addNodeBtn:getY() + 1)
        :setText("")

    instance.deleteNodeBtn = instance.nodeDetailsFrame:addButton()
        :setPosition(instance.nodeDetailsFrame:getWidth() - 7, instance.addNodeBtn:getY()) -- Adjusted for turtle
        :setSize(5, 1)                                                                     -- Reduced from 8 to 5
    ...
    instance.NewTriggerBtn = instance.frame:addButton()
        ...
        :setSize(1, 1) -- Single character button
        :setBackground(colors.red)
        :setForeground(colors.white)
        :setText("N")
        :onClick(function()
    ...
    instance.saveTriggerBtn = instance.frame:addButton()
        :setPosition(instance.NewTriggerBtn:getX() + instance.NewTriggerBtn:getWidth() + 1,
            instance.conditionTree:getY() + instance.conditionTree:getHeight() + 1)
        :setSize(1, 1) -- Single character button
        :setBackground(colors.green)
        :setForeground(colors.white)
        :setText("S")
    ...
    instance.cancelBtn = instance.frame:addButton()
        :setPosition(instance.saveTriggerBtn:getX() + instance.saveTriggerBtn:getWidth() + 1,
            instance.conditionTree:getY() + instance.conditionTree:getHeight() + 1)
        :setSize(1, 1) -- Single character button
        :setBackground(colors.gray)
        :setForeground(colors.white)
    ...
    -- 创建可重复使用的MessageBox实例 - adjusted for turtle
    instance.messageBox = MessageBox:new(instance.frame, 20, 8) -- Reduced from 30x10 to 20x8

    -- 创建可重复使用的ConfirmMessageBox实例 - adjusted for turtle
    instance.confirmMessageBox = ConfirmMessageBox:new(instance.frame, 20, 8) -- Reduced from 30x10 to 20x8

    -- Add scrollbar to nodeDetailsFrame like in RecipeTab
    ScrollableFrame.setScrollable(instance.nodeDetailsFrame, true, colors.gray, colors.lightGray, colors.gray,
        colors.white)
    ...
    self.itemListBox:open(listItems, false, {
        confirm = function(selectedItems)
            ...
            if selectedNode.id == "root" then
                ...
                self.displayedParentNodelabel:setText(StringUtils.ellipsisMiddle(self.selectNodeData.parentNodeName,
                    self.displayedParentNodelabel:getWidth()))
            end
        end
    })
end
    ...
    self.itemListBox:open(listItems, false, {
        confirm = function(selectedItems)
            if selectedItems[1].data then
                ...
                self.selectedItemLabel:setText(StringUtils.ellipsisMiddle(self.selectNodeData.itemName,
                    self.selectedItemLabel:getWidth()))
            end
        end
    })
end
...
function TriggerView:updateDetails(nodeData)
    if nodeData == nil then
        ...
        self.selectedItemLabel:setText(StringUtils.ellipsisMiddle(self.selectNodeData.itemName or "",
            self.selectedItemLabel:getWidth()))
